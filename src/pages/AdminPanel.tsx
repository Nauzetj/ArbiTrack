...
import { getAllUsers, getAllPromoCodes, createPromoCode, deletePromoCode, updateUserRole, getAllPaymentRequests, updatePaymentRequestStatus } from '../services/dbOperations';
...
export const AdminPanel: React.FC = () => {
  ...
  const handleGenerateCode = () => {
    ...
    else if (newCodeDuration === '12m') expires.setFullYear(expires.getFullYear() + 1);

    const newCode: PromoCode = {
      id: generateUUID(),
      code: generateCode(),
      plan: newCodeDuration === '12m' ? 'vip_annual' : newCodeDuration === '6m' ? 'vip_semiannual' : 'vip_monthly',
      createdAt: now.toISOString(),
      expiresAt: expires.toISOString(),
      usedAt: null,
      usedBy: null,
    };
    createPromoCode(newCode);
    reload();
  };

  const handleApproveRequest = (req: PaymentRequest) => {
    // Generate a promo code for this request
    const now = new Date();
    const expires = new Date(now);
    expires.setDate(expires.getDate() + 15); // code redemption window

    const code = generateCode();
    const promoId = generateUUID();
    const plan = req.duration === '12m' ? 'vip_annual' : req.duration === '6m' ? 'vip_semiannual' : 'vip_monthly';
    ...
    updatePaymentRequestStatus(req.id, 'approved', reviewNote || null, code);
    setApprovedCode(code);
    reload();
    setReviewingReq({ ...req, status: 'approved', generatedCode: code });
  };

  const handleRejectRequest = (req: PaymentRequest) => {
    updatePaymentRequestStatus(req.id, 'rejected', reviewNote || null, null);
  ...
  const handleChangeRole = (userId: string, role: UserRole) => {
    ...
    updateUserRole(userId, role, planExpiresAt);
    reload();
  };
  ...
  return (
    <div className="flex flex-col gap-[24px] max-w-[1100px] mx-auto pb-[40px] animate-fade-in-up">
      ...
      <div className="flex items-center justify-between">
        ...
        <button onClick={reload} className="flex items-center gap-[6px] px-[12px] py-[7px] text-[13px] text-[var(--text-secondary)] bg-[var(--bg-surface-3)] border border-[var(--border-strong)] rounded-[8px] hover:bg-[var(--bg-surface-4)] transition-colors">
                ...
                return (
                  <tr key={u.id} className="border-b border-[var(--border)] hover:bg-[var(--bg-surface-3)] transition-colors">
                    ...
                    <td className="py-[14px] px-[20px]">
                      <Badge variant={PLAN_BADGE_VARIANT[u.role] || 'neutral'}>
                    ...
                    <td className="py-[14px] px-[20px] text-[12px] text-[var(--text-tertiary)]">
          ...
          <div className="bg-[var(--bg-surface-2)] rounded-[16px] border border-[var(--border)] p-[24px]">
            ...
            <div className="flex flex-wrap items-end gap-[12px]">
              ...
              <div className="flex flex-col gap-[6px]">
                ...
                <div className="flex gap-[8px]">
                  {([
                    ...
                    <button
                      key={d.id}
                      onClick={() => setNewCodeDuration(d.id)}
              ...
              <button
                ...
                className="flex items-center gap-[8px] px-[20px] py-[9px] bg-[var(--accent)] text-white font-semibold text-[13px] rounded-[8px] hover:opacity-90 transition-all hover:-translate-y-[1px] shadow-[0_2px_8px_var(--accent-muted)] hover:shadow-[0_4px_12px_var(--accent-border)] self-end"
              >
                <Plus size={14} /> Generar Código
              </button>
            </div>
            <p className="text-[11px] text-[var(--text-tertiary)] mt-[14px]">
              {newCodeDuration === '15d' && 'El código dará acceso promocional por 15 días a partir de su uso.'}
              {newCodeDuration === '1m'  && 'El código dará acceso completo por 1 mes a partir de su uso.'}
              {newCodeDuration === '6m'  && 'El código dará acceso completo por 6 meses a partir de su uso.'}
              {newCodeDuration === '12m' && 'El código dará acceso completo por 12 meses a partir de su uso.'}
              {' '}El código en sí expira (no puede usarse) después de 15 días si no se canjea.
            </p>
          </div>
                ...
                {codes.length === 0 ? (
                  ...
                  return (
                    <tr key={c.id} className={`border-b border-[var(--border)] transition-colors ${c.usedAt || new Date(c.expiresAt) < new Date() ? 'opacity-50' : 'hover:bg-[var(--bg-surface-3)]'}`}>
                      <td className="py-[14px] px-[20px]">
                        <span className="font-mono text-[15px] font-bold tracking-[2px] text-[var(--text-primary)]">{c.code}</span>
                      </td>
                      <td className="py-[14px] px-[20px]">
                        <div className="flex flex-col gap-[3px]">
                          <Badge variant={c.plan === 'vip_annual' ? 'profit' : 'accent'}>
                            {(() => {
                              const diffMs = new Date(c.expiresAt).getTime() - new Date(c.createdAt).getTime();
                      ...
                      <td className="py-[14px] px-[20px]">
                        <span className={`text-[11px] font-medium px-[8px] py-[3px] rounded-full ${s.bg} ${s.color}`}>
                          {s.label}
                        </span>
                      </td>
                      <td className="py-[14px] px-[20px] text-[12px] text-[var(--text-tertiary)]">
      ...
      {tab === 'requests' && (
        <div className="flex flex-col gap-[16px]">
          {requests.length === 0 ? (
            <div className="bg-[var(--bg-surface-2)] rounded-[16px] border border-[var(--border)] p-[48px] text-center text-[var(--text-secondary)] text-[14px]">
              No hay solicitudes de pago aún.
            </div>
          ) : (
            <div className="grid grid-cols-1 gap-[16px]">
              {requests.map(req => {
                ...
                return (
                  <div key={req.id} className={`bg-[var(--bg-surface-2)] rounded-[16px] border transition-all ${
                    isPending ? 'border-[var(--warning)]/40' : isApproved ? 'border-[var(--profit)]/30' : 'border-[var(--border)] opacity-60'
                  }`}>
                    <div className="flex items-start gap-[20px] p-[20px]">
                      {/* Thumbnail */}
                      <button
                        onClick={() => { setReviewingReq(req); setReviewNote(''); setApprovedCode(req.generatedCode); }}
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-[12px]">
                          ...
                          <span className={`text-[11px] font-bold px-[8px] py-[3px] rounded-full flex-shrink-0 ${
                            isPending ? 'bg-[var(--warning-bg)] text-[var(--warning)]'
                            : isApproved ? 'bg-[var(--profit-bg)] text-[var(--profit)]'
                            : 'bg-[var(--bg-surface-4)] text-[var(--text-tertiary)]'
                          }`}>
                        ...
                        <div className="flex items-center gap-[12px] mt-[6px] flex-wrap">
                          <span className="text-[12px] text-[var(--text-secondary)]">
                            {req.duration === '15d' ? '🎁 Promo 15 días'
                              : req.duration === '1m' ? '⚡ 1 Mes'
                              : req.duration === '6m' ? '🔥 6 Meses'
                              : '🌟 12 Meses'}
                          </span>
                        ...
                        {req.reviewNote && (
                          <p className="mt-[8px] text-[12px] text-[var(--text-tertiary)] italic">"{req.reviewNote}"</p>
                        )}
                      </div>
                      ...
                      {isPending && (
                        <div className="flex flex-col gap-[6px] flex-shrink-0">
                          <button
                            onClick={() => { setReviewingReq(req); setReviewNote(''); setApprovedCode(null); }}
                ...
                <div className="flex flex-col gap-[12px]">
                  <div className="flex flex-col gap-[6px]">
                    ...
                    <input
                      ...
                      placeholder="Ej: Pago verificado por Binance Pay"
                  ...
                  <div className="flex gap-[10px]">
                    <button
                      onClick={() => handleApproveRequest(reviewingReq)}
                    ...
                    <button
                      onClick={() => handleRejectRequest(reviewingReq)}
