-- ============================================================
-- FIX: Matemáticas de ganancia para ciclos parciales
...
BEGIN
  ...
  -- Ganancias en base al volumen realmente emparejado
  v_matched_vol := CASE 
    WHEN v_usdt_vendido < v_usdt_recomprado THEN v_usdt_vendido 
    ELSE v_usdt_recomprado 
  END;
  
  v_ganancia_ves  := (v_matched_vol * v_diferencial);
  v_ganancia_usdt := CASE 
    WHEN v_tasa_compra > 0 THEN (v_ganancia_ves / v_tasa_compra) - v_comision_total
